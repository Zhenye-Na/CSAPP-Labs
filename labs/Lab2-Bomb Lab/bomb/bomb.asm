...
/*
 * First bomb:
 *     compare input with string $0x402400, if not equals, then explode.
 *     so `exec-file bomb` and `x /s 0x402400` (Examine a string stored at 0x402400),
 *     we get "Border relations with Canada have never been better."
 *
 */
...
/*
 * Second bomb:
 *     Check out line 877 for function <read_six_numbers>: read in six numbers.
 *     The first number should be 1, otherwise <explode_bomb> is called.
 *         400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)"
 *         400f0e:	74 20                	je     400f30 <phase_2+0x34>
 *         400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>
 *     Then the remaining numbers should be "2 * former number"
 *
 *     So the array of numbers should be "1 2 4 8 16 32"
 */
 ...
 *     0x4025cf:	"%d %d" => %esi
 *
 *     This bomb is a switch table consists of jump table with 8 entries.
 *
 *     400f60:	83 f8 01             	cmp    $0x1,%eax
 *     400f63:	7f 05                	jg     400f6a <phase_3+0x27>
 *     400f65:	e8 d0 04 00 00       	callq  40143a <explode_bomb>
 *     So %eax should always greater than 0x1
 *
 *     400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8)
 *     jump to &(*0x402470 + 8 * %rax), get the value.
 *
 *     We can use gdb to inspect:
 *     (gdb) x /8a 0x402470
 *     0x402470:	0x400f7c	0x400fb9
 *     0x402480:	0x400f83	0x400f8a
 *     0x402490:	0x400f91	0x400f98
 *     0x4024a0:	0x400f9f	0x400fa6
 *
 *     So the solution should be one of the tuples below:
...
0000000000400fce <func4>:
  ...
  400fd6:	89 c1                	mov    %eax,%ecx ;ecx = 14 - esi
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx ;shr sets the high-order bit to 0.
  400fdb:	01 c8                	add    %ecx,%eax ; 0 or 1
  400fdd:	d1 f8                	sar    %eax       ;the high-order bit remains unchanged
 ...
 *     400ff7:	39 f9                	cmp    %edi,%ecx
 *     400ff9:	7d 0c                	jge    401007 <func4+0x39>
 *     => it will return 0x0 iff %edi == %ecx, meanwhile from <phase_4>,
 *        tells us return value should always be 0x0.
 *
 *     400fe2:	39 f9                	cmp    %edi,%ecx
 *     400fe4:	7e 0c                	jle    400ff2 <func4+0x24>
 *     400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx
 *     400fe9:	e8 e0 ff ff ff       	callq  400fce <func4>
 *     => Recursion should always in this branch, so:
 *        (0 0), (1 0), (3 0), (7 0)
 *
 */
...
000000000040100c <phase_4>:
  ...
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp) ;0xe=14
 ...
 *     401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi
 *     401048:	e8 81 ff ff ff       	callq  400fce <func4>
 *     => func4(0x8(%rsp), 0x0, 0xe)
 *
 *     40104d:	85 c0                	test   %eax,%eax
 *     40104f:	75 07                	jne    401058 <phase_4+0x4c>
 *     => %eax must be 0x0, otherwise program will explode bomb.
 *
 *     401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp)
 *     401056:	74 05                	je     40105d <phase_4+0x51>
 *     401058:	e8 dd 03 00 00       	callq  40143a <explode_bomb>
 *     => %rcx (0xc(%rsp)) must equal to 0x0, otherwise program will explode bomb.
 *
 */
 ...
 *     and the code is performing a stack-guard check
 *
 *     (gdb) x /s 0x4024b0
 ...
 *     4010a8:	48 83 f8 06          	cmp    $0x6,%rax
 *     4010ac:	75 dd                	jne    40108b <phase_5+0x29>
 *     => This is a while loop, until (%rax == 0x6)
 *
 *     Get the lower 4 bits of input as shifting bits, using string saved at 0x4024b0,
 *     to retrieve the same string at 0x40245e. If the two strings are the same,
 *     then safe.
 *
 */
 ...
 * <read_six_numbers>
 *     input,         // $rdi
 *     0x4025c3,      // $esi  // "x /s 0x4025c3" => "%d %d %d %d %d %d"
 *     arr,           // $rdx
 *     arr + 0x4,     // $rcx
 *     arr + 0x8,     // $r8
 *     arr + 0xc,     // $r9
